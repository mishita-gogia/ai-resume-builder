...
if (generatedData.references) {
  ...
  const renderNavbar = () => {
    ...
    <header className="sticky top-0 z-50 w-full border-b border-white/5 bg-slate-950/30 backdrop-blur-3xl shadow-[0_0_40px_rgba(34,211,238,0.08)]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          >
           <div className="w-10 h-10 rounded-2xl bg-gradient-to-br from-cyan-500 via-indigo-500 to-violet-600 flex items-center justify-center shadow-[0_0_30px_rgba(6,182,212,.35)] transition-all duration-500 group-hover:scale-110 group-hover:rotate-6">
              <Mic className="w-4.5 h-4.5 text-white" />
            </div>
            <span className="text-xl font-black tracking-tight text-white font-display">
              VoiceCV<span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-indigo-500 font-extrabold font-display">.AI</span>
            </span>
          </div>
  ...
  const renderHomeView = () => {
    return (
      <div className="flex flex-col gap-16 py-10 antialiased">
        {/* Hero Area */}
        <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center space-y-5 pt-6 pb-8 relative">
  ...
  Empowered by Sarvam AI & NVIDIA AI
          ...
          <h1 className="text-5xl sm:text-6xl lg:text-7xl font-black text-white tracking-tight max-w-4xl mx-auto leading-[1.05] font-display">
            ATS-Ready Resumes,{" "}
        ...
        <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              { value: "47,200+", label: "Resumes Generated", icon: "📄" },
              { value: "93%", label: "Avg. ATS Score", icon: "🎯" },
              { value: "22 Languages", label: "Voice Supported", icon: "🎙️" },
              { value: "4.9 / 5.0", label: "User Rating", icon: "⭐" },
            ].map((stat, i) => (
              <div key={i} className="glass-panel rounded-2xl p-5 text-center space-y-1 border border-white/10">
                <div className="text-xl mb-1">{stat.icon}</div>
                <div className="text-2xl font-black text-white font-display gradient-text">{stat.value}</div>
  ...
  const renderBuilderView = () => {
    return (
      <>
      <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-6 flex flex-col gap-6 pb-36">
        
        {/* Workspace header bar */}
        <div className="glass-panel p-3.5 rounded-xl shadow-sm flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
          <div className="flex items-center gap-3 w-full sm:w-auto">
            <LayoutTemplate className="w-5 h-5 text-cyan-400 shrink-0" />
            <input
              type="text"
              value={resumeTitle}
              onChange={(e) => setResumeTitle(e.target.value)}
              className="text-sm font-bold text-white bg-transparent border-b border-dashed border-white/10 focus:border-cyan-400 focus:outline-none w-full sm:max-w-xs font-display"
            />
          </div>
          <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto justify-end">
            {selectedLang !== "en-IN" && selectedLang !== "en-US" && (translatedResumeData || isTranslatingLocal) && (
              <div className="flex items-center gap-1 bg-cyan-500/10 border border-fuchsia-500/20 p-1 rounded-lg shrink-0">
                <button type="button" onClick={() => setShowTranslated(false)} className={`px-2.5 py-1 text-[11px] font-bold rounded-md transition cursor-pointer ${!showTranslated ? "bg-white/10 text-cyan-300" : "text-slate-500 hover:text-slate-200"}`}>English</button>
                <button type="button" onClick={() => setShowTranslated(true)} disabled={isTranslatingLocal} className={`px-2.5 py-1 text-[11px] font-bold rounded-md transition cursor-pointer flex items-center gap-1 ${showTranslated ? "bg-gradient-to-r from-cyan-500 to-fuchsia-500 text-white" : "text-slate-500 hover:text-slate-200"}`}>
                  {isTranslatingLocal ? <><Loader2 className="w-3 h-3 animate-spin" /> Localizing...</> : <span>Regional ({selectedLang.split("-")[0].toUpperCase()})</span>}
                </button>
              </div>
            )}
            <span className="text-[10px] text-slate-600 hidden sm:block">|</span>
            <button onClick={createNewResumeHandler} className="bg-white/5 hover:bg-white/10 text-slate-300 text-xs font-semibold px-3 py-1.5 rounded-lg cursor-pointer flex items-center gap-1.5"><Trash2 className="w-3 h-3" /> Clear</button>
            <button onClick={saveResumeToDatabase} disabled={isSavingResume} id="btn-save-database" className="bg-gradient-to-r from-cyan-500 to-fuchsia-500 hover:from-cyan-400 hover:to-fuchsia-400 active:scale-95 text-white font-bold text-xs px-4 py-1.5 rounded-lg glow-cyan cursor-pointer flex items-center gap-1.5">
              {isSavingResume ? <><Loader2 className="w-3.5 h-3.5 animate-spin" /> Saving...</> : <><Bookmark className="w-3.5 h-3.5" /> Save to MongoDB</>}
            </button>
          </div>
        </div>

        {/* NEW Split Layout: Left=Controls/Actions, Right=Preview+ATS */}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
          
          {/* ── LEFT PANEL: All action controls ── */}
          <div className="space-y-5">

            {/* ── Action Buttons Panel ── */}
            <div className="glass-panel p-4 rounded-2xl shadow-sm space-y-3">
              <span className="text-[10px] font-bold uppercase tracking-widest text-slate-500 block">Quick Actions</span>
              <div className="grid grid-cols-2 gap-3">
                <button
                  onClick={saveResumeToDatabase}
                  disabled={isSavingResume}
                  className="bg-gradient-to-br from-cyan-500/20 to-fuchsia-500/20 border border-cyan-400/30 hover:border-cyan-400/60 text-white text-xs font-bold py-3 px-3 rounded-xl flex flex-col items-center gap-1.5 transition cursor-pointer"
                >
                  <Bookmark className="w-4 h-4 text-cyan-400" />
                  {isSavingResume ? "Saving..." : "Save to MongoDB"}
                </button>
                <button
                  onClick={() => {
                    const previewComp = document.querySelector("[data-download-btn]") as HTMLButtonElement;
                    if (previewComp) previewComp.click();
                  }}
                  className="bg-gradient-to-br from-emerald-500/20 to-cyan-500/20 border border-emerald-400/30 hover:border-emerald-400/60 text-white text-xs font-bold py-3 px-3 rounded-xl flex flex-col items-center gap-1.5 transition cursor-pointer"
                >
                ...
                <button
                  onClick={createNewResumeHandler}
                  className="bg-white/5 border border-white/10 hover:border-white/20 text-slate-300 text-xs font-bold py-3 px-3 rounded-xl flex flex-col items-center gap-1.5 transition cursor-pointer"
                >
                ...
                <button
                  ...
                  className="bg-white/5 border border-white/10 hover:border-violet-400/30 text-slate-300 text-xs font-bold py-3 px-3 rounded-xl flex flex-col items-center gap-1.5 transition cursor-pointer"
                >
            ...
            {/* ── Template Selector ── */}
            <div className="glass-panel p-4 rounded-2xl shadow-sm space-y-3">
              <span className="text-[10px] font-bold uppercase tracking-widest text-slate-500 block">Template Options</span>
              <div className="grid grid-cols-3 gap-2.5">
                {[
                  { id: ResumeTemplate.MODERN, label: "Modern Split", desc: "Two column" },
                  { id: ResumeTemplate.PROFESSIONAL, label: "Centered Classic", desc: "Academic" },
                  { id: ResumeTemplate.MINIMAL, label: "Minimalist", desc: "High-contrast" }
                ].map((temp) => (
                  <button
                    key={temp.id}
                    onClick={() => setTemplateId(temp.id)}
                    className={`p-3 rounded-xl border text-left transition cursor-pointer ${templateId === temp.id ? "border-cyan-400 bg-cyan-500/10 shadow-sm" : "border-white/10 hover:border-cyan-400/40"}`}
                  >
          ...
          {/* ── RIGHT PANEL: Resume Preview + ATS Score ── */}
          <div className="space-y-5">
  ...
  return (
    <div className="min-h-screen bg-transparent flex flex-col justify-between font-sans text-slate-200 relative overflow-x-hidden antialiased">
      ...
      <div className="absolute bottom-[20%] left-[-10%] w-[500px] h-[500px] bg-indigo-500/[0.07] rounded-full blur-3xl opacity-70 z-0 pointer-events-none"></div>
      <div className="absolute top-[40%] right-[-10%] w-[400px] h-[400px] bg-violet-500/[0.06] rounded-full blur-3xl opacity-60 z-0 pointer-events-none"></div>
